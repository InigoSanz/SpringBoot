...
import com.iem.web.resttemplate.Dato;
import com.iem.web.resttemplate.ServicioDatos;
...
public class ControladorGestorPersonas {
	...
	public ResponseEntity<String> getPost() {
		...
		String salida = servicioDatos.postDato(dato);
	...
	@GetMapping("/servicios/get")
	public ResponseEntity<String> getGet() {

		String salida = servicioDatos.getDato();
	...
	@GetMapping("/servicios/put")
	public ResponseEntity<String> getPut() {

		Dato dato = new Dato();
		dato.setDato1("Datos para el PUT");

		String salida = servicioDatos.putDato(dato);
	...
	@GetMapping("/servicios/delete")
	public ResponseEntity<String> getDelete() {

		String salida = servicioDatos.deleteDato();
	...
	// Patch (parcial) y Put (total), uno deja nulls y el otro no ¿? ... Revisar
	// esto !!¡¡
...
//		}
	...
	@GetMapping("/pacientes/{id-paciente}/medicos")
	public ResponseEntity<List<String>> getMedicosDePacientes(@PathVariable("id-paciente") String idPaciente) {

		log.debug("Obtener médicos de un paciente");

		for (int i = 0; i < pacientes.size(); i++) {
			if (pacientes.get(i).getIdentificador().equalsIgnoreCase(idPaciente)) {

				List<String> salida = pacientes.get(i).getCitasPendiente().stream()
						.map(citaPendiente -> citaPendiente.getIdentificadorMedico()).toList();
